...
import { getUniCourses, getUniCoursesCount } from './lib/db'
...
const {query, page} = await searchParams
    ...
    const uniCourses = await getUniCourses(query, pageNumber) as UniversitiesCourses[]
    
    const rows = []
    for (let i = 0; i < uniCourses.length; i++) {
        const statusColor = uniCourses[i].course_status === Status.pending ? "bg-yellow-200" : uniCourses[i].course_status === Status.approved ? "bg-green-200" : "bg-red-200"
        rows.push(<tr key={i} className='hover:bg-[#eee] duration-300 border-b-2 border-b-gray-100'>
            ...
            <td className='table-custom-cell'><span className={`${statusColor} px-2 py-1 rounded-lg`}>{uniCourses[i].course_status}</span></td>
            <td className='table-custom-cell flex justify-center'><Link href={uniCourses[i].syllabus} className='w-fit flex justify-center items-center'><File className='duration-300 w-10 h-10 p-2 hover:bg-blue-400 hover:text-white rounded-lg'></File></Link></td>
            ...
            <div className='rounded-md shadow-custom p-3 overflow-x-auto'>
                <div className='mb-3 flex justify-between'>
                    ...
                    <div className='flex gap-2 items-center'>
                        <Search url='/admission/requests' text='search for a request'></Search>
                    </div>
                </div>
