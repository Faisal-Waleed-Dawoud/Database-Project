...
import { Roles, signInFormErrors, signUpFormErrors, User } from "@/lib/types"
import { insertUser, userExists } from "../users"
...
export async function signUp(prevState: signUpFormState, formData: FormData) {
    ...
    if (fName.match(/[\p{Emoji_Presentation}\p{Emoji}\uFE0F]/ug)) {
        errors.firstName = "First Name Cannot have emojies"
    }

    if (lName.match(/[\p{Emoji_Presentation}\p{Emoji}\uFE0F]/ug)) {
        errors.lastName = "Last Name Cannot have emojies"
    }
